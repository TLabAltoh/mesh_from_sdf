...
class SDF2MESH_OT_Apply_Gizmo_To_SDF_GLSL(Operator):
    bl_idname = 'mesh_from_sdf.apply_gizmo_to_sdf_glsl'
    bl_label = 'Apply Gizmo to SDF GLSL'
    bl_description = 'Reflect Gizmo changes in SDFGLSLProperty'
    
    def execute(self, context):
        global bound
        
        glsl_pointer = context.scene.sdf_glsl_pointer_list[context.object.sdf_prop.sub_index]
        glsl_pointer.bound = bound
        ...
        SDFGLSLPointer.update_glsl_mesh(pointer)
        ...
        # bpy.ops.ed.undo_push(message='mesh_from_sdf.apply_gizmo_to_sdf_glsl')
        return {'FINISHED'}
...
class SDFGLSLWidgetGroup(GizmoGroup):
    bl_idname = "OBJECT_GGT_sdf_glsl"
    bl_label = "SDF GLSL Widget"
    ...
    def poll(cls, context):
        return context.object and context.object.sdf_prop.enabled and (context.object.sdf_prop.primitive_type == 'GLSL')
    ...
    def setup(self, context):
        ...
        def move_get_bound_x():
            value = bpy.context.scene.sdf_glsl_pointer_list[context.object.sdf_prop.sub_index].bound[0]
            # Apparently, it is not possible to write a value to sdf_glsl_gizmo_prop in the setup function.
            # This function is called when an object is selected, so update the value of sdf_glsl_gizmo_prop here.
            bound[0] = max(0, value)
            return value

        def move_set_x(value):
            bound[0] = max(0, value)
            bpy.ops.mesh_from_sdf.apply_gizmo_to_sdf_glsl()
            
        def move_get_bound_y():
            value = bpy.context.scene.sdf_glsl_pointer_list[context.object.sdf_prop.sub_index].bound[1]
            bound[1] = max(0, value)
            return value
        ...
        def move_get_bound_z():
            value = bpy.context.scene.sdf_glsl_pointer_list[context.object.sdf_prop.sub_index].bound[2]
            bound[2] = max(0, value)
            return value
        ...
        # pointer = bpy.context.scene.sdf_glsl_pointer_list[ob.sdf_prop.sub_index]
        # bound = pointer.bound
...
classes = [
    SDF2MESH_OT_Apply_Gizmo_To_SDF_GLSL,
    SDFGLSLWidgetGroup
]
