...
class SDF2MESH_OT_Apply_Gizmo_To_SDF_Hex_Prism(Operator):
    bl_idname = 'mesh_from_sdf.apply_gizmo_to_sdf_hex_prism'
    bl_label = 'Apply Gizmo to SDF Hexagonal Prism'
    bl_description = 'Reflect Gizmo changes in SDFHexPrismProperty'
    
    def execute(self, context):
        global height, radius
        hex_prism_pointer = context.scene.sdf_hex_prism_pointer_list[context.object.sdf_prop.sub_index]
        hex_prism_pointer.height = height * 2.0
        hex_prism_pointer.radius = radius
        ...
        SDFPrismPointer.update_hex_prism_mesh(pointer)
        ...
        # bpy.ops.ed.undo_push(message='mesh_from_sdf.apply_gizmo_to_sdf_hex_prism')
        return {'FINISHED'}
...
class SDFHexPrismWidgetGroup(GizmoGroup):
    bl_idname = "OBJECT_GGT_sdf_hex_prism"
    bl_label = "SDF Hexagonal Prism Widget"
    ...
    def poll(cls, context):
        return context.object and context.object.sdf_prop.enabled and (context.object.sdf_prop.primitive_type == 'Hexagonal Prism')
    ...
    def setup(self, context):
        ...
        def move_get_height():
            global height
            value = bpy.context.scene.sdf_hex_prism_pointer_list[context.object.sdf_prop.sub_index].height * 0.5
            # Apparently, it is not possible to write a value to sdf_hex_prism_gizmo_prop in the setup function.
            # This function is called when an object is selected, so update the value of sdf_hex_prism_gizmo_prop here.
            height = max(0, value)
            return value
        ...
        def move_set_height(value):
            global height
            height = max(0, value)
            bpy.ops.mesh_from_sdf.apply_gizmo_to_sdf_hex_prism()
            
        def move_get_radius():
            global radius
            value = bpy.context.scene.sdf_hex_prism_pointer_list[context.object.sdf_prop.sub_index].radius
            radius = max(0, value)
            return value
...
#        pointer = bpy.context.scene.sdf_hex_prism_pointer_list[ob.sdf_prop.sub_index]
...
classes = [
    SDF2MESH_OT_Apply_Gizmo_To_SDF_Hex_Prism,
    SDFHexPrismWidgetGroup
]
